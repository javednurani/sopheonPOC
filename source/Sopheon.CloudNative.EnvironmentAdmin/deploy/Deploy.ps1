...
$ResourceGroup = "Stratus-$($Environment))";

$SqlAdminEngima = (az keyvault secret show --vault-name "Stratus-$($Environment)" --name "SqlServerAdminEnigma" --query value).Replace('"', '');
...
Invoke-Sqlcmd -ServerInstance "$($ResourceGroup.ToLower()).database.windows.net" -Database $ResourceGroup -UserName "sopheon" -Password $SqlAdminEngima -InputFile "_StratusEnvironmentManagement\EnvironmentManagement\scripts.sql";
Write-Host "Complete!";
