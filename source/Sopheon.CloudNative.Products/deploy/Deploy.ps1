...
& $ZipUtil "a" "-tzip" "$env:Build_ArtifactStagingDirectory\ProductManagement-Staged" $ProductManagementPath


#upload webapp app
az webapp deployment source config-zip --name $WebApiAppServiceName --resource-group $ResourceGroup --src "ProductManagement-Staged\ProductManagement.zip";