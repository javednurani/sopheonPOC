...
Copy-Item -Path "$($env:System_DefaultWorkingDirectory)\source\webapps\modules\app\deploy\*" -Destination "$($env:Build_ArtifactStagingDirectory)";
...
& $ZipUtil "a" "-tzip" "$($env:Build_ArtifactStagingDirectory)\App_$($env:Build_BuildId)" "$($env:System_DefaultWorkingDirectory)\source\webapps\modules\app\dist\*" "-xr!build" "-xr!deploy";
